 ...
 * available for the current allocation context.
 */
...
#define LRU_GEN_MAX_SWAPPINESS	200

#define evictable_min_seq(min_seq, swappiness)				\
	min((min_seq)[!(swappiness)],					\
	    (min_seq)[(swappiness) != LRU_GEN_MAX_SWAPPINESS])
...
#define for_each_evictable_type(type, swappiness)			\
	for ((type) = !(swappiness);					\
	     (type) <= ((swappiness) != LRU_GEN_MAX_SWAPPINESS);		\
	     (type)++)
...
static int should_skip_vma(unsigned long start, unsigned long end, struct mm_walk *args)
{
	...
	if (walk->swappiness == LRU_GEN_MAX_SWAPPINESS)
		return true;
...
static bool inc_min_seq(struct lruvec *lruvec, int type, int swappiness)
{
	...
	if (type ? swappiness == LRU_GEN_MAX_SWAPPINESS : !swappiness)
		goto done;
...
static void try_to_inc_min_seq(struct lruvec *lruvec, int swappiness)
{
	...
	for_each_evictable_type(type, swappiness) {
		...
		for (seq = lrugen->min_seq[type];
		     seq + MIN_NR_GENS <= lrugen->max_seq; seq++) {
...
static int get_type_to_scan(struct lruvec *lruvec, int swappiness)
{
	...
	if (swappiness == LRU_GEN_MAX_SWAPPINESS)
		return LRU_GEN_ANON;
	/*
	...
	read_ctrl_pos(lruvec, LRU_GEN_FILE, MAX_NR_TIERS,
		      LRU_GEN_MAX_SWAPPINESS - swappiness, &pv);
			 ...
			 int *isolate_type, int *isolate_scanned)
{
	...
	*isolated = 0;
	for_each_evictable_type(i, swappiness) {
		...
		/* Keep the selected type when it made scan progress. */
		if (!scanned)
			type = !type;
	}
		       ...
		       bool *need_swapping)
{
	...
	/* In case page deletion left empty old generations, flush them. */
	try_to_inc_min_seq(lruvec, swappiness);

	scanned = isolate_pages(nr_to_scan, lruvec, sc, swappiness, &list,
				&isolated, &type, &type_scanned);

	/* Scanning may have emptied the oldest generation, flush it. */
	if (scanned)
		try_to_inc_min_seq(lruvec, swappiness);
...
static int lru_gen_seq_show(struct seq_file *m, void *v)
{
	...
	if (!full)
		seq = evictable_min_seq(min_seq, LRU_GEN_MAX_SWAPPINESS / 2);
		     ...
		     int swappiness, bool force_scan)
{
	...
	return try_to_inc_max_seq(lruvec, max_seq, sc, swappiness, force_scan) ?
	       0 : -EEXIST;
}
			...
			int swappiness, unsigned long nr_to_reclaim)
{
	...
	while (!signal_pending(current)) {
		...
		if (!evict_pages(min(nr_to_reclaim - sc->nr_reclaimed,
				    (unsigned long)MAX_LRU_BATCH),
				 lruvec, sc, swappiness, NULL))
			return 0;
