...
next:
	...
	/* keep the two oldest evictable generations within MIN_NR_GENS */
	if (swappiness && swappiness <= LRU_GEN_MAX_SWAPPINESS) {
			     ...
			     int swappiness, bool force_scan)
{
	...
	if (!force_scan && evictable_min_seq(min_seq, swappiness) + MAX_NR_GENS - 1 <= max_seq)
		return -ERANGE;

	try_to_inc_max_seq(lruvec, max_seq, sc, swappiness, force_scan);
		   ...
		   struct scan_control *sc, int swappiness, unsigned long opt)
{
	...
	if (swappiness < 0)
		swappiness = get_swappiness(lruvec, sc);
	else if (swappiness > LRU_GEN_MAX_SWAPPINESS + 1)
		goto done;
